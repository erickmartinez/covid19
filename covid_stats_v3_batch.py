...
input_csv = './fitting_table2.csv'
results_folder = './SEIR_v3_batch_20200412'
...
before_date = datetime.datetime(year=2020, month=4, day=12)
...
seir_fit_dtype = np.dtype([
        ...
        ('inflection_point3', 'i'),
        ('inflection_point4', 'i'),
        ('inflection_date1', 'M8[ns]'),
        ('inflection_date2', 'M8[ns]'),
        ('inflection_date3', 'M8[ns]'),
        ('inflection_date4', 'M8[ns]'),
        ('before_day', 'i8'),
        ('fit date' , 'M8[ns]'),
        ('beta1_guess', 'd'),
        ('gamma1_guess', 'd'),
...
if __name__ == '__main__':
    ...
    for _, row  in df_input.iterrows():
        ...
        if location == 'China' or location == 'Australia':
            add_days = 90
        else:
            add_days = 365
            
        start_idx = int(row['start_idx'])
#        before_day = int(row['before_day'])
       
        inflection_point1 = int(row['inflection_point1'])
        inflection_point2 = int(row['inflection_point2'])
        inflection_point3 = int(row['inflection_point3'])
        inflection_point4 = int(row['inflection_point4'])
        ...
        if inflection_point3 > inflection_point2:
            ...
            res3 = optimize.least_squares(fobj_seir, np.log10(popt2),#[popt2_log[0], popt2_log[1], np.log10(e_[-1])],#np.log10(np.array(b1)), 
                                          jac='3-point',
                                          bounds=(b_lower_limit,[10,10,np.log10(population)]),
    ...
    #    pcov_seir = np.power(10, pcov1_log)
        ...
        per100k_factor = 1E5/population
        
        if plot_pbands:
            ax2.fill_between(time_pred1,lpb1[:,0]*per100k_factor,upb1[:,0]*per100k_factor, color=pband_color)
            ax2.fill_between(time_pred1,lpb1[:,1]*per100k_factor,upb1[:,1]*per100k_factor, color=pband_color2)
            ax2.fill_between(time_pred2,lpb2[:,0]*per100k_factor,upb2[:,0]*per100k_factor, color=pband_color)
            ax2.fill_between(time_pred2,lpb2[:,1]*per100k_factor,upb2[:,1]*per100k_factor, color=pband_color2)
        ax2.plot(time_days_fit, infected_fit*per100k_factor,'o', color=data_color, fillstyle='none', label='infected')
        ax2.plot(time_days_fit, removed_fit*per100k_factor,'v', color=removed_color, fillstyle='none', label='removed')
        ax2.plot(time_pred1, ypred1[:,0]*per100k_factor, ':', color=model2_color)
        ax2.plot(time_pred1, ypred1[:,1]*per100k_factor, ':', color=removed_color)
        ax2.plot(time_pred2, ypred2[:,0]*per100k_factor, color=model2_color)
        ax2.plot(time_pred2, ypred2[:,1]*per100k_factor, color=removed_color)
        if inflection_point3 > inflection_point2:
            if plot_pbands:
                ax2.fill_between(time_pred3,lpb3[:,0]*per100k_factor,upb3[:,0]*per100k_factor, color=pband_color)
                ax2.fill_between(time_pred3,lpb3[:,1]*per100k_factor,upb3[:,1]*per100k_factor, color=pband_color2)
            ax2.plot(time_pred3, ypred3[:,0]*per100k_factor,'--', color=model2_color)
            ax2.plot(time_pred3, ypred3[:,1]*per100k_factor, '--',color=removed_color)
    
        ax2.annotate('1', xy=(time1[-1], infected1[-1]*per100k_factor), xytext=(0, 20), color='k',
                     xycoords='data', textcoords='offset points', fontsize=8,
                     arrowprops=dict(arrowstyle="->", color='r', connectionstyle="angle3,angleA=0,angleB=90"))
        ax2.annotate('2', xy=(time2[0], infected2[0]*per100k_factor), xytext=(0, -20), color='k',
                     xycoords='data', textcoords='offset points', fontsize=8,
                     arrowprops=dict(arrowstyle="->", color='r', connectionstyle="angle3,angleA=0,angleB=90"))
        ax2.annotate('2', xy=(time2[-1], infected2[-1]*per100k_factor), xytext=(0, -20), color='k',
                     xycoords='data', textcoords='offset points', fontsize=8,
                     arrowprops=dict(arrowstyle="->", color='r', connectionstyle="angle3,angleA=0,angleB=90"))
        if inflection_point3 > inflection_point2:
            ax2.annotate('3', xy=(time3[0], infected3[0]*per100k_factor), xytext=(0, 20), color='k',
                         xycoords='data', textcoords='offset points', fontsize=8,
                         arrowprops=dict(arrowstyle="->", color='r', connectionstyle="angle3,angleA=0,angleB=90"))
    ...
    #                       engfmt.format_data(imax_seir))
        ...
        ax3.plot(time_projected, yprojected[:,2]*per100k_factor, ls='-', color=data_color, label='infected')
        ax3.plot(time_projected, yprojected[:,3]*per100k_factor, ls='--', color=removed_color, label='removed')
        ax3.plot(time_projected, confirmed_seir*recovery_rate*per100k_factor, color='tab:green', label='recovered')
    #    ax3.plot(time_projected, confirmed_seir*death_rate*per100k_factor, color='tab:red', label='dead')
        ...
        projected_infections = infected_res*per100k_factor
    ...
    #            xy=(tmax_seir, imax_seir*per100k_factor), xycoords='data',
    ...
    #         zorder=6)
        ...
        ax3_info_str = 'Population: {0}'.format(engfmt(population))
        ax3_info_str += "\nLethality rate = {0:.3f} %".format(death_rate*100)
    ...
    #    ax4.set_yscale('log')
        ...
        if location == 'China'  or location == 'global' or location == 'Iran':
            ax1.set_ylim(
                    top=np.amax(confirmed)*200, 
        ...
        ax2.set_ylim(
                top=np.amax(ypred2[:]*per100k_factor)*10, 
                bottom=max(1E-5,np.amin(ypred1[:]*per100k_factor))
                )
        
        if inflection_point3 > inflection_point2:
            ax2.set_ylim(
                top=np.amax(ypred3[:]*per100k_factor)*10, 
                bottom=max(1E-5,np.amin(ypred1[:]*per100k_factor))
                )
    ...
    #    ax2.xaxis.set_minor_locator(mticker.AutoMinorLocator(4))
        ...
        if np.amax(np.log10(yprojected[:,2])) - np.amin(np.log10(yprojected[:,2])) > 2:
            if location == 'China':
                ax3.set_ylim(bottom=max(1E-4,np.amin(yprojected[:,2]*per100k_factor*death_rate)),
                             top=np.amax(yprojected[:,2])*per100k_factor*80)
            else:
                ax3.set_ylim(bottom=max(1E-4,np.amin(yprojected[:,2]*per100k_factor*death_rate)),
                             top=np.amax(yprojected[:,2])*per100k_factor*10)
            ax3.set_yscale('log')
            ax3.yaxis.set_major_locator(locmaj2)
            ax3.yaxis.set_minor_locator(locmin2)
        else:
            ax3.set_ylim(bottom=max(-1E-4,np.amin(yprojected[:,2]*per100k_factor*death_rate)*0.75),
                         top=np.amax(yprojected[:,2])*per100k_factor*1.8)
    ...
    #    ax3.yaxis.set_minor_locator(mticker.AutoMinorLocator(2))
        ...
        if np.amax(np.log10(mild_infections)) - np.amin(np.log10(mild_infections)) > 2:
            ax4.set_ylim(bottom=max(1E-5,np.amin(critical_infections_intervened)),top=np.amax(mild_infections)*5)
            ax4.set_yscale('log')
            ax4.yaxis.set_major_locator(locmaj3)
            ax4.yaxis.set_minor_locator(locmin3)
        else:
            ax4.set_ylim(bottom=max(-0.01,np.amin(critical_infections_intervened)*0.05),top=np.amax(mild_infections)*1.5)
    ...
    #    ax4.yaxis.set_minor_locator(mticker.AutoMinorLocator(4))
        ...
        ax1_d.xaxis.set_major_locator(day_locator)
        ...
        ax1.yaxis.tick_left()
        ax1_k_lim = (
                    ax1.get_ylim()[0]*1E5/population,
                    ax1.get_ylim()[1]*1E5/population
                )
        ax1_k = ax1.twinx()
    ...
    #        popt2[2], ci2[2][0], ci2[2][1]))
        ...
        if save_results:
            ...
            fig.savefig(os.path.join(results_folder,filetag+'.eps'), dpi=600, format='eps')
    #        fig_pop.savefig(os.path.join(results_folder,filetag_pop+'.png'), dpi=600)
            ...
            fitting_results = np.array([(
                        ...
                        np.datetime64(t0_fit) + np.timedelta64(inflection_point1,'D'),
                        np.datetime64(t0_fit) + np.timedelta64(inflection_point2,'D'),
                        np.datetime64(t0_fit) + np.timedelta64(inflection_point3,'D'),
                        np.datetime64(t0_fit) + np.timedelta64(inflection_point4,'D'),
                        before_day,
                        ...
                        confirmed[-1], recovered[-1], dead[-1], population,
                        beds_per_k, popt2[0]/popt1[0], popt3[0]/popt2[0]
                    )], dtype=seir_fit_dtype)
            new_results_df = pd.DataFrame(data=fitting_results)
            if not os.path.exists(results_path):
                new_results_df.to_csv(path_or_buf=results_path, index=False, date_format='%Y-%m-%d %H:%M:%S')
            else:
